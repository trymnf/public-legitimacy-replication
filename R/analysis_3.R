...
# Extract a helper dataset with all hits (dimension == "none")

# Leveraging the power of join! Feels like the smoothest way to do this 
...
ags_frac_yearly %>% 
  ...
  pivot_wider(names_from = dimension, values_from = frac_hits, id_cols = c(year, agency_excel, all_arts)) %>%
  mutate(leg_prevalence = leg_command - evidence) %>% 
  na.exclude()-> for_fe_no_drop

ags_frac_yearly %>% 
  dplyr::select(-c(frac_hits)) %>% 
  pivot_wider(names_from = dimension, values_from = count, id_cols = c(year, agency_excel, all_arts)) %>%
...
parlq <- parlq %>% 
  mutate(document_reference = str_trim(document_reference), 
         year = get_year(year))
...
for_fe_no_drop %>% 
  left_join(yearly_parlqs, by = c("agency_excel" = "agency", "year")) %>% 
  filter(agency_excel != "ERA") %>% 
  # We know that agencies with NA on parl_questions have no questions in that year. 
  mutate(parl_questions = ifelse(is.na(parl_questions), 0, parl_questions)) -> for_fe_parl_no_drop

for_mod_counts_no_drop %>% 
  left_join(yearly_parlqs, by = c("agency_excel" = "agency", "year")) %>%  
  filter(agency_excel != "ERA") %>% 
  # We know that agencies with NA on parl_questions have no questions in that year. 
  mutate(parl_questions = ifelse(is.na(parl_questions), 0, parl_questions)) -> for_mod_parl_counts_no_drop
...
to_plot_top <- yearly_ags %>% 
  ...
  arrange(desc(count)) %>% 
  mutate(agency_excel = fct_reorder(agency_excel, count))

to_plot_top %>%
  ggplot(aes(count, agency_excel, color = agency_excel %in% highlights)) + 
...
parlq %>% 
  ...
  mutate(across(c("evidence", "participation", "hr"), function (x) x + 1)) -> mod_all_counts2
...
to_boot %>% 
  ...
  pivot_wider(names_from = dimension,
              values_from = dim_sum) -> to_boot_agency
...
get_mean_boots_agency <- function(split){
  analysis(split) %>%
    pivot_longer(cols = -c(agency_excel, all_arts, hardness, external_operational, jha),
                 names_to = "dimension",
                 values_to = "dim_sum") %>% 
    group_by(hardness, dimension) %>% 
    summarise(dim = sum(dim_sum), 
              tot = sum(all_arts),
              n = n(), .groups = "drop") %>% 
...
boot_mod_ungr %>%  
  mutate(hardness = as_factor(hardness)) %>% 
  ggplot(aes(hardness, fill = hardness,
             y = prop_dim)) +
  ...
  stat_slab(shape = 21,
            ...
            scale = 3,
            alpha = 0.5) +
  geom_errorbar(data = summary_bootmod,
                aes(hardness, estimate,
                    ymin = ci_lower,
                    ymax = ci_upper),
                width = 0.1) +
  geom_point(data = summary_bootmod,
             aes(hardness, estimate,
                 fill = hardness),
             shape = 21,
...
to_boot_agency %>%
  pivot_longer(cols = c(evidence:participation),
               names_to = "dimension",
               values_to = "count") %>% 
  group_by(jha, dimension) %>% 
  summarise(m_count = mean(count),
            s_count = sum(count),
            all_arts = sum(all_arts), 
            n = n()) %>% 
  mutate(w_mean = m_count*all_arts/sum(all_arts),
  ...
  filter(dimension == "hr") %>% 
  ggplot(aes(jha, prop)) +
  # ggbeeswarm::geom_quasirandom(data = filter(bees, 
  #                                            dimension == "hr"), 
  #                              aes(jha, 
  #                                  prop_indiv, 
  #                                  group = jha),
  ...
  geom_point(shape = 21,
             ...
             aes(fill = jha),
             show.legend = FALSE) + 
  scale_x_discrete(labels = c("0" = "**Not** MHA", 
                              "1" = "MHA")) + 
  labs(x = NULL,
       y = "Proportion of articles") + 
  scale_fill_brewer(palette = "Set1") + ylim(0, NA)
