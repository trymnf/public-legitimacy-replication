...
We used a script for parsing text files from the Retriever Mediearkivet database written in `Python` by [anonymized for peer review].
We ran the script from `R` through the `reticulate` package [@R-reticulate].
This gave us a dataset with `r nrow(howmany)` articles and `r ncol(howmany)` variables.
...
Figure \@ref(fig:aic-over-time) shows the monthly percentage of articles in an agency's context that also mentions the agency.
